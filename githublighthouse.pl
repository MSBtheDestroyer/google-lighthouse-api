...
my $original_url = shift @ARGV;
unless ($original_url) {
	die "Usage: $0 [--mobile|-m] <domain>\n";
}
...
# Determine the strategy based on the mobile flag
my $strategy = $mobile ? "mobile" : "desktop";

# Google Endpoint
my $api_endpoint = "https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=$site_url&key=$api_key&strategy=$strategy";
...
if ($response->is_success) {
	...
	if (exists $data->{lighthouseResult}{audits}) {
		...
		foreach my $metric (@metrics) {
			...
			if ($metric->{key} eq 'network-requests') {
				...
				if (defined $audits->{'total-byte-weight'}->{numericValue}) {
					$display_value = sprintf(
						"%.2f MB",
						$audits->{'total-byte-weight'}->{numericValue} / (1024 * 1024)
					);
				} else {
					$display_value = 'N/A';
				}
				...
				if (defined $audits->{'unminified-css'}->{details}->{overallSavingsBytes}) {
					# Convert from bytes to KB
					my $savings_bytes = $audits->{'unminified-css'}->{details}->{overallSavingsBytes};
				...
				# Check if the overallSavingsBytes value is defined
				if (defined $audits->{'unminified-javascript'}->{details}->{overallSavingsBytes}) {
					# Convert from bytes to KB
					my $savings_bytes = $audits->{'unminified-javascript'}->{details}->{overallSavingsBytes};
					my $savings_kb    = $savings_bytes / 1024;
					$display_value = sprintf("Potential Savings %.2f KB", $savings_kb);
				} else {
					$display_value = 'N/A';
				...
				# Check if the overallSavingsBytes value is defined
				if (defined $audits->{'unused-javascript'}->{details}->{overallSavingsBytes}) {
					# Convert from bytes to KB
					my $savings_bytes = $audits->{'unused-javascript'}->{details}->{overallSavingsBytes};
					my $savings_kb    = $savings_bytes / 1024;
					$display_value = sprintf("Potential Savings %.2f KB", $savings_kb);
				} else {
					$display_value = 'N/A';
				...
				# Check if the overallSavingsBytes value is defined
				if (defined $audits->{'unused-css-rules'}->{details}->{overallSavingsBytes}) {
					# Convert from bytes to KB
					my $savings_bytes = $audits->{'unused-css-rules'}->{details}->{overallSavingsBytes};
					my $savings_kb    = $savings_bytes / 1024;
					$display_value = sprintf("Potential Savings %.2f KB", $savings_kb);
				} else {
					$display_value = 'N/A';
				...
				# Check if the overallSavingsBytes value is defined
				if (defined $audits->{'uses-responsive-images'}->{details}->{overallSavingsBytes}) {
					# Convert from bytes to KB
					my $savings_bytes = $audits->{'uses-responsive-images'}->{details}->{overallSavingsBytes};
					my $savings_kb    = $savings_bytes / 1024;
					$display_value = sprintf("Potential Savings %.2f KB", $savings_kb);
				} else {
					$display_value = 'N/A';
			...
			my $metric_label = $metric->{label} // ucfirst(join ' ', map { lc } split /-/, $metric->{key});
		...
		if ($mobile) {
			say "\n Testing URL for Mobile: $original_url";
		} else {
			say "\n Testing URL for Desktop: $original_url";
		}
		print $tb;
